...
- A stale, abandoned connect/disconnect attempt's native callback can still resolve whichever attempt is live when it eventually arrives; generation correlation is out of scope (see Decision).
