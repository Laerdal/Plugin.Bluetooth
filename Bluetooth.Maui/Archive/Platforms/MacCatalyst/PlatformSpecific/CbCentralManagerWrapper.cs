...
public partial class CbCentralManagerWrapper : CBCentralManagerDelegate
{
    ...
    public CbCentralManagerWrapper(CbCentralManagerWrapper.ICbCentralManagerProxyDelegate scanner, IOptions<BluetoothAppleOptions>? options = null)
    {
        _scanner = scanner;
        options ??= new OptionsWrapper<BluetoothAppleOptions>(new BluetoothAppleOptions());
        CbCentralManager = new CBCentralManager(this, options.Value.GetCentralQueue(), options.Value.CBCentralInitOptions)
        {
            Delegate = this
        };
    }
