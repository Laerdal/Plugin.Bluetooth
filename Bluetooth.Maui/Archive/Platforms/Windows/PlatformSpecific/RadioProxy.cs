...
public partial class RadioProxy
{
    ...
    /// </summary>
    /// <param name="bluetoothAdapter">The Bluetooth adapter to get the radio from.</param>
    /// <param name="radioProxyDelegate">The delegate for handling radio events.</param>
    /// <returns>A task that represents the asynchronous operation. The task result contains the radio proxy instance.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="bluetoothAdapter"/> is null.</exception>
    ...
    public async static Task<RadioProxy> GetInstanceAsync(Windows.Devices.Bluetooth.BluetoothAdapter bluetoothAdapter, IRadioProxyDelegate radioProxyDelegate)
    {
        ArgumentNullException.ThrowIfNull(bluetoothAdapter);
        RadioInstance ??= await bluetoothAdapter.GetRadioAsync();
        return RadioInstance == null ? throw new PermissionException("BluetoothAdapter.GetRadioAsync = null, Did you forget to add '<DeviceCapability Name=\"bluetooth\" />' in your Manifest's Capabilities ?") : new RadioProxy(RadioInstance, radioProxyDelegate);
    }
