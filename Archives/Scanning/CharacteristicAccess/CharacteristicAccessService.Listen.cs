...
public abstract partial class CharacteristicAccessService<TRead, TWrite>
{
    ...
    /// <inheritdoc />
    public async ValueTask SubscribeAsync(IBluetoothDevice device, IBluetoothCharacteristicAccessService<TRead, TWrite>.OnNotificationReceived listener, TimeSpan? timeout = null, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async ValueTask UnsubscribeAsync(IBluetoothDevice device, IBluetoothCharacteristicAccessService<TRead, TWrite>.OnNotificationReceived listener, TimeSpan? timeout = null, CancellationToken cancellationToken = default)
    {
        var characteristic = await GetCharacteristicAsync(device, timeout, cancellationToken).ConfigureAwait(false);
        ...
        if (!isSubscribed)
        {
            // TODO : LOG : WARN - Listener already unsubscribed
            return;
        }
    ...
    /// <inheritdoc />
    public async ValueTask UnsubscribeAllAsync(IBluetoothDevice device, TimeSpan? timeout = null, CancellationToken cancellationToken = default)
    {
        var characteristic = await GetCharacteristicAsync(device, timeout, cancellationToken).ConfigureAwait(false);

        lock (_listenersLock)
        {
            _listeners.RemoveAll(tuple => tuple.Item1.Service.Device.Equals(device));
        }
        await UpdateListeningStateAsync(characteristic).ConfigureAwait(false);
    }
    ...
    public IEnumerable<(IBluetoothCharacteristic, IBluetoothCharacteristicAccessService<TRead, TWrite>.OnNotificationReceived)> GetListeners(IBluetoothDevice device)
    {
        lock (_listenersLock)
        {
            return _listeners.Where(l => l.Item1.Service.Device == device).ToList();
        }
    }
    ...
    public IEnumerable<IBluetoothCharacteristicAccessService<TRead, TWrite>.OnNotificationReceived> GetListeners(IBluetoothCharacteristic characteristic)
    {
        lock (_listenersLock)
        {
            return _listeners.Where(l => l.Item1 == characteristic).Select(l => l.Item2).ToList();
        }
    }

    /// <inheritdoc />
    public bool HasListeners(IBluetoothDevice device)
    {
        return GetListeners(device).Any();
    }

    /// <inheritdoc />
    public bool HasListeners(IBluetoothCharacteristic characteristic)
    {
        return GetListeners(characteristic).Any();
    }

    /// <inheritdoc />
    public int GetListenerCount(IBluetoothDevice device)
    {
        return GetListeners(device).Count();
    }
