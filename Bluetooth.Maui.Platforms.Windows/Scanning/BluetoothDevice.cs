    ...
    GattSessionWrapper.IGattSessionDelegate
{
        ...
        : base(scanner, request, serviceFactory, rssiToSignalStrengthConverter)
    {
        ...
        if (request is not BluetoothDeviceFactoryRequest)
        {
            throw new ArgumentException(
                $"Expected request of type {typeof(BluetoothDeviceFactoryRequest)}, but got {request.GetType()}");
        }
    }
    ...
    {
        var serviceRequest = new BluetoothServiceFactoryRequest(nativeService);
